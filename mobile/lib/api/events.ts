...
export async function createEvent(
  ...
  const event = await handler<BackendEventRecord>("/api/v1/events", {
    method: "POST",
    auth: true,
...
export async function updateEvent(
  eventId: string,
  patch: Partial<Pick<EventRecord, "title" | "event_date" | "event_time" | "event_timezone" | "event_location" | "event_type" | "configuration" | "require_rsvp_approval">>,
): Promise<EventRecord> {
...
export async function publishEventDesign(
  eventId: string,
  layers: CanvasLayer[],
  configuration: FlyerConfiguration,
): Promise<EventRecord> {
  const event = await handler<BackendEventRecord>(`/api/v1/events/${eventId}/publish`, {
    method: "POST",
    auth: true,
    json: { layers: normalizeCanvasLayers(layers), configuration },
  });
  const normalized = normalizeEvent(event);
  setCachedQuery(`event:${eventId}`, normalized, 60_000);
  return normalized;
}

export async function returnEventToEditor(eventId: string): Promise<EventRecord> {
  const event = await handler<BackendEventRecord>(`/api/v1/events/${eventId}/return-to-editor`, {
    method: "POST",
    auth: true,
  });
  const normalized = normalizeEvent(event);
...
export function createMockEvent(
  ...
  return {
    ...
    design_status: overrides.design_status ?? "published",
    design_published_at: Object.prototype.hasOwnProperty.call(overrides, "design_published_at")
      ? overrides.design_published_at ?? null
